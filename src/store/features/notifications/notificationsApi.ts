...
export const notificationsApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getNotifications: builder.query<
      { data: Notification[]; meta: { total: number; page: number; limit: number } },
    ...
    >({
      ...
      transformResponse: (response: { data: { data: Notification[]; meta: any } }) => response.data,
      providesTags: (result) =>
        result
          ? [
    ...
    // Update notification
    updateNotification: builder.mutation<Notification, { id: string; data: UpdateNotificationInput }>({
      query: ({ id, data }) => ({
    ...
    sendBroadcastNotification: builder.mutation<void, { title: string; message: string; type: string }>({
      query: (body) => ({
